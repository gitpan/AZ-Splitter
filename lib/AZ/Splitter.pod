...
=head1 SYNOPSIS
  ...
  $s = AZ::Splitter->new(\*IN,$limitsize);
  $s = AZ::Splitter->new(\*IN,$limitsize,$buffsize);
  $s = AZ::Splitter->new(\*IN,$limitsize,$buffsize,s_2BUFF|s_UTFOK);
  $s = AZ::Splitter->new(\$SCALAR,...); # Same for scalar variables
...
STREAM_IN - is a reference to file stream, opened for reading,
or reference to string.

SIZE_LIMIT - limit size of input stream data in characters. If this
argument is not defined or equal to -1, then all input stream
data will be available for read.

BUFF_SIZE - size of buffer in characters. If this argument is not
...
s_UTFOK - disable utf8 data check in UTF-8 mode. Use this flag if
...
STREAM_OUT - is a reference to file stream, opened for writing,
or reference to string. If another type of value (ex: undef), then
data will be not stored.

SEPARATORS - is a string-separator or reference to array with
many separators.

MAX_SIZE - size in characters. Defines, the maximum number of
characters that must be stored in STREAM_OUT. If this argument not
...
s_APPEND - append data to STREAM_OUT if STREAM_OUT is a reference
to string.
...
MAX_SIZE - limit size in characters, how many it is necessary
...
All flags exist in AZ:: namespace and you can disable import
...
=head2 Find one of substrings in file without case sensitive:

  my(@strings,$stream,$fh,$result);

  # Initialize array of strings
  @strings=("word1","word2","phrase 1","word3");

  open($fh,"<","file.txt") or die $!;
  $stream = AZ::Splitter->new($fh);

  # Now, let trying to find one of substrings
  $result = $stream->read_to(undef,\@strings,-1,s_WCASE|s_WEND);
